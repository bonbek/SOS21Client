package ddgame.client.triggers {
	...
	import ddgame.client.view.components.Panel;
	import ddgame.client.view.components.Button;
...
//						trace("Quiz déjà joué @" + toString());
		...
		*/
		...
		private function callForData():void
		{
			...
			if (quizId > 0)
			{
				ApplicationChannel.getInstance().addEventListener(PublicServerEventList.ON_DATAQUIZ, dataQuizHandler);
				sendPublicEvent(new BaseEvent(PublicServerEventList.GET_DATAQUIZ, quizId));
			}
			else
			{
				if (sourceTarget)
