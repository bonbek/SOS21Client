		...
		override public function execute(event:Event = null):void
		{
			if (isPropertie("target") && isPropertie("tile")) {
				...
				if (tar.length == 3)
				{
					...
					var ta:AbstractTile = AbstractTile.getTile(tar[0]);
					if (tileTa == "bob") {
						sendEvent(new BaseEvent(EventList.MOVE_PLAYER_TO_TILE, ta));
					} else {
						trace("Tile ", tileTa);
						sendEvent(new BaseEvent(EventList.MOVE_TILE_TO_TILE, {tile:tileTm, targetTile:ta} ));
					}
				}
