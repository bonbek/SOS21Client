package ddgame.client.commands {
	 ...
	 *	@author toffer
	 */
	public class MoveTileToTileCommand extends Notifier implements ICommand {
		...
		public function execute(event:Event):void
		{
			...
			var tile:AbstractTile = AbstractTile(BaseEvent(event).content.targetTile);
			var otile:AbstractTile = AbstractTile(BaseEvent(event).content.tile);
			...
			if(!gridProxy.isCollisionCell(tilepos))
			{
				...
				sendEvent(new BaseEvent(EventList.MOVE_TILE, {tile:otile, cellTarget:tilepos}));
			} else {
				...
				// on recup le point le plus proche du tile
				tpoint = gridProxy.findNearestNoCollisionPoint(tilepos);
				// hop hop, allez on bouge
				sendEvent(new BaseEvent(EventList.MOVE_TILE, {tile:otile, cellTarget:tpoint}));
			}*/
		}
