package ddgame.proxy {
	...
	public class LibProxy extends AbstractProxy {
		...
		public function load():void
		{
			...
			_lib.load();
			sendPublicEvent(new Event(EventList.GFXLIB_UPDATESTART));
		}
		...
		private function glibHandler(event:GLibEvent):void
		{
			switch (event.type)
			{
				...
				{
					sendPublicEvent(new BaseEvent(EventList.GFXLIB_PROGRESS, int(event.bytesLoaded / event.bytesTotal * 100)));
					break;
				}
				case GLibEvent.ON_LIB_COMPLETE :
				{
					...
					sendPublicEvent(new Event(EventList.GFXLIB_COMPLETE));
					break;
				}
			}
		}
