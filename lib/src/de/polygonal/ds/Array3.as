...
package de.polygonal.ds
{
	...
	public class Array3 implements Collection
	{
		...
		{
//			trace( "get cell: " + _a[int((z * _w * _h) + (y * _w) + x)] + " " + (int((z * _w * _h) + (y * _w) + x)) + " - x:" + x + " y:" + y + " z:" +z );
...
//			trace( "set cell: "+ (int((z * _w * _h) + (y * _w) + x)) + " - x:" + x + " y:" + y + " z:" +z );
		}
		...
		{
			trace( "_a.length: " + _a.length );
			for ( var c:int = 0; c < _a.length; c++ ) {
				if (_a[c] is CollisionCell) {
					trace( c + " : CollisionCell");
				} else {
//					trace("bob");
				}
			}
...
//			trace( tmp );
			...
			if (al == 0)
				return;
// TODO notifier polygonal labs bug Array3.resize
/*			_a.length = 0;
			_a.length = w * h * d;
	...
	trace( "_a.length: " + _a.length );
			...
			_h = h;
			_d = d;
//			trace("resize: " + _w + "," + _h + "," + _d );
			for ( var i:int = 0; i < _a.length; i++ ) {
				if (_a[i] is CollisionCell) trace( i + " : CollisionCell");
			}
		}
