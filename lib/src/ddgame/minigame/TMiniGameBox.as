package ddgame.minigame {
	...
	public class TMiniGameBox extends MiniGameUIContainer {
		...
		public function TMiniGameBox (w:int = 0, h:int = 0, styles:Object = null) : void
		{
			super(w, h, styles);
		...
		override public function get height () : Number
		{
			...
			if (h <= 0)
			{
				...
				if (_styles.direction == VERTICAL) {
					h = _childsHeight + whiteSpacesHeight;
					trace("VERTICAL", _childsHeight, whiteSpacesHeight);
				} else {
					h = bigestChildHeight  + _styles.paddingTop + _styles.paddingBottom;
					trace("HORIZONTAL", bigestChildHeight, _styles.paddingTop, _styles.paddingBottom);
				}
			}
			else
			{
